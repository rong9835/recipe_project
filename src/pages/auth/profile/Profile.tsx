			...
			<div className={styles.profileBox}>
				<section className={styles.profile}>
					<div className={styles.user}>
						...
						<h2>{userNickname}<img src="./src/assets/icon_nicknameImg.png" alt='' /></h2>
						{
							editIntroduction ? (
								<div className={styles.userEditIntroduction}>
									<input type="text" value={userIntroduction || ""} onChange={(e) => setUserIntroduction(e.target.value)} />
									<button onClick={userIntroductionUpdate}>저장</button>
									<button onClick={() => setEditIntroduction(false)}>취소</button>
								</div>
							) : (
								<span onClick={() => setEditIntroduction(true)}>{userIntroduction ? userIntroduction : "자기소개를 작성해주세요."}<img src="./src/assets/icon_infoImg.png" alt='' /></span>
							)
						}
					</div>
					...
					!showEditInfo && !showEditPassword ? (
						...
						<section className={styles.userInformation}>
							...
							<div className={styles.userInfo}>
								...
								<div>
									<button onClick={() => setShowEditInfo(true)}>정보수정<img src="./src/assets/icon_profileButton.png" alt="" /></button>
									<button onClick={() => setShowEditPassword(true)}>비밀번호 변경<img src="./src/assets/icon_profileButton.png" alt="" /></button>
				...
				}
				{
					showEditInfo && !showEditPassword ? (
						...
						<section className={styles.userEditInfo}>
							<div className={styles.editBack} onClick={() => setShowEditInfo(false)}>
								<img src="./src/assets/icon_infoback.png" alt="" />
								<span>뒤로가기</span>
							</div>
							<div className={styles.editInput}>
								<span>이메일</span>
								<input type="email" className={styles.editInputEmail} readOnly value={userEmail || ""} />
							</div>
							<div className={styles.editInput}>
								<span>이름</span>
							...
							<div className={styles.editInput}>
								<span>연락처</span>
								<input type="tel" value={userPhoneNumber || ""} onChange={(e) => setUserPhoneNumber(e.target.value)} />
							</div>
				...
				}
				{
					!showEditInfo && showEditPassword ? (
						// 비밀번호 변경 화면
						<section className={styles.userEditInfo}>
							<div className={styles.editBack} onClick={() => setShowEditPassword(false)}>
								<img src="./src/assets/icon_infoback.png" alt="" />
								<span>뒤로가기</span>
							</div>
							<div className={styles.editInput}>
